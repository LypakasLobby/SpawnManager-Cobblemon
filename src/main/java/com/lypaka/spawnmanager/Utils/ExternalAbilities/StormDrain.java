...
public class StormDrain {
    ...
    public static boolean applies (Pokemon pokemon) {
        ...
        return pokemon.getAbility().getName().equalsIgnoreCase("StormDrain") || pokemon.getAbility().getName().equalsIgnoreCase("Storm Drain");
    ...
    public static Pokemon tryStormDrain (Pokemon originalSpawn, Map<PokemonSpawn, Double> possibleSpawns) {
        ...
        for (Map.Entry<PokemonSpawn, Double> entry : possibleSpawns.entrySet()) {
            ...
            if (entry.getKey().getTypes().contains(ElementalTypes.INSTANCE.getWATER())) {
