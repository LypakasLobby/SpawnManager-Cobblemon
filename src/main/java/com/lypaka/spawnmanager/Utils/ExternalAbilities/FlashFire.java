...
public class FlashFire {
    ...
    public static boolean applies (Pokemon pokemon) {
        ...
        return pokemon.getAbility().getName().equalsIgnoreCase("FlashFire") || pokemon.getAbility().getName().equalsIgnoreCase("Flash Fire");
    ...
    public static Pokemon tryFlashFire (Pokemon originalSpawn, Map<PokemonSpawn, Double> possibleSpawns) {
        ...
        for (Map.Entry<PokemonSpawn, Double> entry : possibleSpawns.entrySet()) {
            ...
            if (entry.getKey().getTypes().contains(ElementalTypes.INSTANCE.getFIRE())) {
