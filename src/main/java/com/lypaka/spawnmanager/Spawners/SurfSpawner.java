...
import com.lypaka.spawnmanager.SpawnAreas.Spawns.SurfSpawn;
                ...
                if (currentSpawnArea.getSurfSpawnerSettings().getBlockIDs().contains(blockID)) {
                    ...
                    if (p.containsKey(area)) {
                        ...
                        if (uuids.contains(player.getUuid())) {
                            ...
                            if (count.get() >= currentSpawnArea.getSurfSpawnerSettings().getSpawnLimit()) continue;

                            List<PokemonEntity> finalPlayersSpawnedPokemon = playersSpawnedPokemon;
                            SpawnManager.spawnerLogicThread.execute(() -> {

                                List<GeneratedSpawn> toSpawn = SpawnGenerator.generateSurfSpawn(area, player);
                                if (toSpawn.isEmpty()) return;
                                if (!currentSpawnArea.getSurfSpawnerSettings().doesAutoBattle()) {
                                    ...
                                    player.getServer().executeSync(() -> {
                                        ...
                                        for (GeneratedSpawn spawn : toSpawn) {
                                            ...
                                            if (count.get() >= currentSpawnArea.getSurfSpawnerSettings().getSpawnLimit()) {

                                                break;

                                            }
                                            playerPokemonCount.put(player.getUuid(), finalPlayersSpawnedPokemon);
                                            if (currentSpawnArea.getSurfSpawnerSettings().doesDespawnAfterBattle()) {
                                            ...
                                            String message = currentSpawnArea.getSurfSpawnerSettings().getMessagesMap().get(messageType);
