...
import com.lypaka.spawnmanager.SpawnAreas.Spawns.CaveSpawn;
                ...
                if (currentSpawnArea.getCaveSpawnerSettings().getBlockIDs().contains(blockID)) {
                    ...
                    if (p.containsKey(area)) {
                        ...
                        if (uuids.contains(player.getUuid())) {
                            ...
                            if (count.get() >= currentSpawnArea.getCaveSpawnerSettings().getSpawnLimit()) continue;

                            List<PokemonEntity> finalPlayersSpawnedPokemon = playersSpawnedPokemon;
                            SpawnManager.spawnerLogicThread.execute(() -> {

                                List<GeneratedSpawn> toSpawn = SpawnGenerator.generateCaveSpawn(area, player);
                                if (toSpawn.isEmpty()) return;
                                if (!currentSpawnArea.getCaveSpawnerSettings().doesAutoBattle()) {
                                    ...
                                    player.getServer().executeSync(() -> {
                                        ...
                                        for (GeneratedSpawn spawn : toSpawn) {
                                            ...
                                            if (count.get() >= currentSpawnArea.getCaveSpawnerSettings().getSpawnLimit()) {

                                                break;

                                            }
                                            playerPokemonCount.put(player.getUuid(), finalPlayersSpawnedPokemon);
                                            if (currentSpawnArea.getCaveSpawnerSettings().doesDespawnAfterBattle()) {
                                            ...
                                            String message = currentSpawnArea.getCaveSpawnerSettings().getMessagesMap().get(messageType);
